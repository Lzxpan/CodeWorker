...
- 不支援新增檔案
- 不支援刪除檔案
- 最穩定的作法仍是先釘選目標檔案，再直接在主對話框提出修改需求
- 若無法唯一定位修改區塊，系統會要求補充上下文或退回保守的文字建議，不會硬猜
- 目前不支援截圖貼上；若建議有錯，請直接用文字在主對話框接著描述問題
