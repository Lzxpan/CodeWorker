...
- 檔案生成：可先預覽再確認寫入 `.txt/.md/.py/.js/.ts/.json/.html/.css/.yaml/.sql/.cs/.docx/.pdf/.pptx`。
...
- Python 文件套件：`pypdf`、`python-docx`、`reportlab`、`python-pptx`
...
1. 開啟專案。
2. 在對話輸入框寫清楚要生成的內容與目標格式，例如：「產生 `docs/spec.md`，內容是登入流程規格」。
3. 點 `生成檔案`。
4. 確認 pending preview 後才會寫入 project root。
...
- 檔案生成與 Agent 寫入都先建立 pending preview，確認後才寫入。
...
- 新增 file generation pending workflow，支援 text/code、`.docx`、`.pdf`、`.pptx`。
- `scripts\bootstrap.ps1` 新增 `reportlab` 與 `python-pptx`。
